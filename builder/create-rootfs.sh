...
buildWithDocker() {
	docker image build -t l4t-builder:1.0 ${root_dir}
	if [[ ${staging} == "yes" ]]; then
		docker run --privileged --cap-add=SYS_ADMIN --rm -it -v ${root_dir}:/root/l4t-fedora l4t-builder:1.0 /root/l4t-fedora/builder/create-rootfs.sh -s
		exit
	fi
	docker run --privileged --cap-add=SYS_ADMIN --rm -it -v ${root_dir}:/root/l4t-fedora l4t-builder:1.0 /root/l4t-fedora/builder/create-rootfs.sh
}
...
prepareFiles() {
	mkdir -p ${build_dir}/{{r,b}ootfs,tmp/,fedora_iso/,switchroot/install/}

	if [[ ! -e ${root_dir}/${archive} ]]; then
		wget ${url} -P ${root_dir}
	fi

	unxz ${root_dir}/${archive}
...
build() {
	...
	if [[ $staging == "yes" ]]; then
		cp -r ${root_dir}/rpmbuilds/*/*.rpm ${root_dir}/rootfs/pkgs/
	fi

	kpartx -a ${build_dir}/"$(basename ${archive} .xz)" && sleep 1
	vgchange -ay fedora && sleep 1
	mount -o loop /dev/mapper/fedora-root ${build_dir}/rootfs/

	# Install Packages
	arch-chroot ${build_dir}/rootfs/ ./build-stage2.sh

	rm -rf ${build_dir}/rootfs/{build-stage2.sh/,pkgs/,usr/bin/qemu-aarch64-static}
	...
	vgchange -an fedora
	kpartx -d ${build_dir}/"$(basename ${archive} .xz)"

	mv ${build_dir}/bootfs/* ${build_dir}/
	dd if=${build_dir}/ of=${root_dir}/l4t-fedora.img bs=4M
}
